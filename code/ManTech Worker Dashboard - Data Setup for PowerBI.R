...
rm(list = ls())
setwd("C:/Users/Paul Liu/Desktop/00. ManTech Skills Mapping")
library(pacman)
p_load(tidyverse)
p_load(tidycensus)
p_load(forcats)
...
occsoc_crosswalk <- read.csv("occsoc_crosswalk_2000_onward_without_code_descriptions.csv") %>% 
...
indnaics_crosswalk <- read.csv("indnaics_crosswalk_2023.csv") %>% 
...
puma_names <- read.xlsx("PUMA Names.xlsx") %>% 
